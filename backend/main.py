...
from backend.recommendation import RecommendationService, RecommendationRequest, RecommendationResponse
...
def health_check(db: Session = Depends(get_db)):
    ...
    try:
        ...
        if settings.LLM_PROVIDER == "ollama":
            import httpx
            r = httpx.get(f"http://{settings.OLLAMA_HOST}:{settings.OLLAMA_PORT}/", timeout=2.0)
...
@cache(namespace=NS_DASHBOARD, ttl_seconds=TTL_DASHBOARD)
def get_dashboard_overview(db: Session = Depends(get_db)):
    """
    Returns a consolidated overview of dashboard KPIs, charts, and timeline lists.
    """
    try:
        service = DashboardService(db)
        return service.get_dashboard_overview()
    except Exception as e:
        logger.error(f"Error compiling dashboard overview: {e}")
        raise HTTPException(status_code=500, detail=str(e))
