...
The platform uses Docker Compose (`docker-compose.yml`) to orchestrate five interconnected services:
...
- Containers resolve each other using service names (e.g., the backend connects to Postgres via host `db:5432`).
...
2. **`ollama`**: Verified using `ollama list || exit 1`.
3. **`backend`**: Verified using `curl -f http://127.0.0.1:8000/api/health || exit 1`.
4. **`frontend`**: Launches only after `backend` is evaluated as healthy.
